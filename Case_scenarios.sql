...
and last name of all employees with a number higher than 109990.
 ...
 is also a manager, according to the data provided in the dept_manager 
...
SELECT
    ...
    CASE  
        WHEN 
			MAX(s.salary) - MIN(s.salary) > 30000 THEN 'Salary was raised by more then $30,000'  
        ELSE 
...
SELECT  
    ...
    MAX(s.salary) - MIN(s.salary) AS salary_difference,  
    IF
		(MAX(s.salary) - MIN(s.salary) > 30000, 'Salary was raised by more then $30,000', 'Salary was NOT raised by more then $30,000') AS salary_increase  
FROM  
    dept_manager dm  
        JOIN  
    employees e ON e.emp_no = dm.emp_no  
...
SELECT
    ...
    CASE
        WHEN 
			MAX(de.to_date) > SYSDATE() THEN 'Is still employed'
        ELSE 
